...
Then your first Claude Code prompt is simply:
```
Read CLAUDE.md fully. Then build the application 
starting with:
1. Global layout with the nav bar exactly as specified
2. Night/day mode toggle wired up with the correct colours
3. The card component matching the metadata format in CLAUDE.md
4. The Search surface with the 9 dummy data items in a 3-column grid

Use the exact fonts, colours, and zero border radius rule. 
Build the fonts in via next/font from Google Fonts.
Do not start on Feed or Map yet.